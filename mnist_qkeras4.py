...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 14 18:25:47 2020
...
model.add(QConv2D(8, (4, 4), strides=(1,1), input_shape=(32,32, 1),
    kernel_quantizer=quantized_bits(14,2),bias_quantizer=quantized_bits(14,2), name="conv2d_0_m"))
...
model.add(QDense(120, name='fc1',
                 kernel_quantizer=quantized_bits(14,2), bias_quantizer=quantized_bits(14,2),
                 kernel_initializer='lecun_uniform', kernel_regularizer=l1(0.0001)))
model.add(QActivation(activation=quantized_relu(14,2), name='relu3'))
model.add(QDense(84, name='fc2',
                 kernel_quantizer=quantized_bits(14,2), bias_quantizer=quantized_bits(14,2),
                 kernel_initializer='lecun_uniform', kernel_regularizer=l1(0.0001)))
model.add(QActivation(activation=quantized_relu(14,2), name='relu4'))
...
if train:
 ...
 #   callbacks.callbacks.append(pruning_callbacks.UpdatePruningStep())
    model.fit(X_train, Y_train, batch_size=1024,
              epochs=50, validation_split=0.25, shuffle=True, callbacks = callbacks.callbacks)
     # Save the model again but with the pruning 'stripped' to use the regular layer types
  #  model = strip_pruning(model)
    model.save('model_6/KERAS_check_best_model.h5')
else:
    from tensorflow.keras.models import load_model
    from qkeras.utils import _add_supported_quantized_objects
    co = {}
    _add_supported_quantized_objects(co)
    model = load_model('model_6/KERAS_check_best_model.h5', custom_objects=co)
...
config = hls4ml.utils.config_from_keras_model(model, granularity='name',
  default_precision='ap_fixed<15,3,AP_RND,AP_SAT>', default_reuse_factor=50)
config['LayerName']['softmax']['exp_table_t'] = 'ap_fixed<18,8>'
config['LayerName']['softmax']['inv_table_t'] = 'ap_fixed<18,4>'

print(config)
hls_model = hls4ml.converters.convert_from_keras_model(model, hls_config=config, output_dir='model_6/hls4ml_prj')
hls_model.compile()
