...
#ifndef _myproject_layer5_out_V_H_
#define _myproject_layer5_out_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "myproject_layer5_out_V_memcore.h"

SC_MODULE(myproject_layer5_out_V) {
    static const unsigned int DataWidth    = 13;
...
protected:
    ...
    myproject_layer5_out_V_memcore* myproject_layer5_out_V_memcore_U;
...
public:
    ...
    SC_CTOR(myproject_layer5_out_V) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        myproject_layer5_out_V_memcore_U = new myproject_layer5_out_V_memcore("myproject_layer5_out_V_memcore_U");
        myproject_layer5_out_V_memcore_U->clk      ( clk );
        myproject_layer5_out_V_memcore_U->reset    ( reset );
        myproject_layer5_out_V_memcore_U->ce0      ( i_ce0 );
        myproject_layer5_out_V_memcore_U->we0      ( i_we0 );
        myproject_layer5_out_V_memcore_U->address0 ( memcore_iaddr );
        myproject_layer5_out_V_memcore_U->d0       ( i_d0 );
        myproject_layer5_out_V_memcore_U->q0       ( i_q0 );

        myproject_layer5_out_V_memcore_U->ce1      ( t_ce0 );
        myproject_layer5_out_V_memcore_U->we1      ( t_we0 );
        myproject_layer5_out_V_memcore_U->address1 ( memcore_taddr );
        myproject_layer5_out_V_memcore_U->d1       ( t_d0 );
        myproject_layer5_out_V_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "myproject_layer5_out_V.trace");
            // ports
