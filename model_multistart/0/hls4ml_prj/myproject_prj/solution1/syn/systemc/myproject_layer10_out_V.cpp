...
myproject_layer10_out_V::~myproject_layer10_out_V() {
    if (m_trace_file) sc_close_vcd_trace_file(m_trace_file);
}

void myproject_layer10_out_V::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void myproject_layer10_out_V::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void myproject_layer10_out_V::proc_memcore_addr() {
    memcore_iaddr = (i_address0.read(), iptr.read());
    memcore_taddr = (t_address0.read(), tptr.read());
}

void myproject_layer10_out_V::proc_push_buf() {
    push_buf.write(i_ce.read() & i_write.read() & full_n.read());
}

void myproject_layer10_out_V::proc_pop_buf() {
    pop_buf.write(t_ce.read() & t_read.read() & empty_n.read());
}

void myproject_layer10_out_V::proc_iptr() {
...
void myproject_layer10_out_V::proc_tptr() {
    if (reset.read() == SC_LOGIC_1) {
        tptr.write(0);
...
void myproject_layer10_out_V::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void myproject_layer10_out_V::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void myproject_layer10_out_V::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
