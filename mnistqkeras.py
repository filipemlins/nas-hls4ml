...
model.add(QConv2D(8, (4, 4), strides=(1,1), input_shape=(32,32, 1),
    kernel_quantizer=quantized_bits(6,0),bias_quantizer=quantized_bits(6,0), name="conv2d_0_m"))
...
model.add(QConv2D(
    16, (2, 2), strides=(1,1),
    kernel_quantizer=quantized_bits(6,0),
    bias_quantizer=quantized_bits(6,0),
    name="conv2d_1_m"))
...
model.add(QDense(120, name='fc1',
                 kernel_quantizer=quantized_bits(6,0), bias_quantizer=quantized_bits(6,0),
                 kernel_initializer='lecun_uniform', kernel_regularizer=l1(0.0001)))
model.add(QActivation(activation=quantized_relu(6), name='relu3'))
model.add(QDense(84, name='fc2',
                 kernel_quantizer=quantized_bits(6,0), bias_quantizer=quantized_bits(6,0),
                 kernel_initializer='lecun_uniform', kernel_regularizer=l1(0.0001)))
model.add(QActivation(activation=quantized_relu(6,0), name='relu4'))
...
#                              outputDir = 'model_3')
 #   callbacks.callbacks.append(pruning_callbacks.UpdatePruningStep())
  ...
  #  model = strip_pruning(model)
    model.save('model_7/KERAS_check_best_model.h5')
else:
    from tensorflow.keras.models import load_model
    from qkeras.utils import _add_supported_quantized_objects
    co = {}
    _add_supported_quantized_objects(co)
    model = load_model('model_7/KERAS_check_best_model.h5', custom_objects=co)
...
config = hls4ml.utils.config_from_keras_model(model, granularity='name',
  default_precision='ap_fixed<7,1,AP_RND,AP_SAT>', default_reuse_factor=50)
config['LayerName']['softmax']['exp_table_t'] = 'ap_fixed<18,8>'
config['LayerName']['softmax']['inv_table_t'] = 'ap_fixed<18,4>'

print(config)
hls_model = hls4ml.converters.convert_from_keras_model(model, hls_config=config, output_dir='model_7/hls4ml_prj')
hls_model.compile()
