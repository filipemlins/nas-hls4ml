...
class VivadoWriter(Writer):
    ...
    def _make_array_pragma(variable):
        """
        Layers in hls_model.py can specify output array ing through the `pragma` attribute.
        ...
        for line in f.readlines():
            ...
            #Add input/output type
                ...
                if model.config.get_config_value("IOType") == "io_parallel":
                    #old for i in model_inputs: newline += indent + self._make_array_pragma(i) + '\n'
                    #old for o in model_outputs: newline += indent + self._make_array_pragma(o) + '\n'
                    ...
                    for var in vars:
                        if var not in inputs and var not in outputs:
                            ...
                            if def_cpp is not None:
                                ...
                                if var.pragma:
                                    newline += '    ' + '\n'
                                    # old newline += '    ' + self._make_array_pragma(var) + '\n'
                    ...
                    if func:
                        for line in func:
                            if firstTime:
                                ...
                                line = line.replace(params['input_t'], 'input_t')
                                line = line.replace(params['input'], 'input1')
                                print(line)
                            newline += '    ' + line + '\n'
#                            print(newline)
